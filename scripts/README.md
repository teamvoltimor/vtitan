...
- **Username** — must be `pi` (the scripts and services assume `/home/pi`)
- **Password** — set one
...
Find the Pi on WiFi (`ssh pi@<hostname>.local` or its `192.168.x.y` lease) and
copy the scripts over. The voldemorbot repo is **private**, so the script clones
it via `gh` and needs GitHub auth for the `pi` user. Two ways:

**A. Token (one pass, non-interactive):**
```bash
scp -r scripts pi@<ip>:/tmp/
ssh pi@<ip> 'sudo GH_TOKEN=ghp_xxx bash /tmp/scripts/setup_pi_5.sh --hailo-deb /path/hailort.deb'
```

**B. Interactive `gh auth login` (two passes):** run the script once — it
installs `gh`, then stops at the auth check. Log in as `pi`, then re-run:
```bash
scp -r scripts pi@<ip>:/tmp/
ssh pi@<ip> 'sudo bash /tmp/scripts/setup_pi_5.sh'   # installs gh, then stops
ssh -t pi@<ip> 'gh auth login'                       # authenticate as pi (one time)
ssh pi@<ip> 'sudo bash /tmp/scripts/setup_pi_5.sh'   # now proceeds to completion
```

Use `setup_pi_zero.sh` for the Pi Zero 2W (USB gadget @ 10.250.250.1) and
`setup_pi_5.sh` for the Pi 5 (usb0 host @ 10.250.250.2). The auth check runs
...
The Pi Zero ↔ Pi 5 link is a USB Ethernet gadget on **10.250.250.0/24**
