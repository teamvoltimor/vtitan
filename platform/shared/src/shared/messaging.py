	...
	def receive(self, topic: str, timeout: float = 1.0) -> Message | None:
		...
		def callback(ros_msg: Any) -> None:
			...
			received_msg = Message(
				topic=topic, data=ros_msg, timestamp=time.time()
			)
	...
	def _create_publisher(self, topic: str, message_type: str | None) -> None:
		...
		if not message_type:
			raise MessagingError(
				f"message_type required for publisher on {topic}"
			)

		try:
			from rosidl_runtime_py.message_to_ordereddict import message_to_ordereddict
			...
			raise MessagingError(
				f"Failed to create publisher on {topic}: {e}"
			) from e
	...
	def _create_subscription(
		...
		try:
			...
			return self._node.create_subscription(
				ros_msg_class, topic, callback, 10
			)
		except Exception as e:
			raise MessagingError(
