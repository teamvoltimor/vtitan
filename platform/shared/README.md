...
- `shared.config.constants` — physical/hardware constants (`RobotSpecs`, `TrackDimensions`,
  `WallSpecs`, `TrafficSignSpecs`). The chassis/Ackermann/LIDAR/camera-mount fields are
  generated from `platform/shared/config/robot.toml` (the actual cross-language source of
  truth — see `robot_constants_gen.py` and `task gen:robot-constants`); everything else in
