...
@pytest.fixture
def robot_model(robot_world: ET.Element) -> ET.Element:
...
class TestCamera:
    def test_camera_link_exists(self, robot_model: ET.Element) -> None:
        assert robot_model.find("link[@name='camera_link']") is not None

    def test_camera_sensor_present(self, robot_model: ET.Element) -> None:
        cam_link = robot_model.find("link[@name='camera_link']")
        assert cam_link.find("sensor[@type='camera']") is not None

    def test_camera_topic(self, robot_model: ET.Element) -> None:
        cam_link = robot_model.find("link[@name='camera_link']")
    ...
    def test_camera_sensor_has_explicit_pose(self, robot_model: ET.Element) -> None:
        cam_link = robot_model.find("link[@name='camera_link']")
        sensor = cam_link.find("sensor[@type='camera']")
        assert sensor.find("pose") is not None

    def test_camera_has_correct_hfov(self, robot_model: ET.Element) -> None:
        cam_link = robot_model.find("link[@name='camera_link']")
        sensor = cam_link.find("sensor[@type='camera']")
    ...
    def test_camera_image_resolution(self, robot_model: ET.Element) -> None:
        cam_link = robot_model.find("link[@name='camera_link']")
        sensor = cam_link.find("sensor[@type='camera']")
...
class TestLidar:
    def test_lidar_link_exists(self, robot_model: ET.Element) -> None:
        assert robot_model.find("link[@name='lidar_link']") is not None

    def test_lidar_sensor_present(self, robot_model: ET.Element) -> None:
        lidar_link = robot_model.find("link[@name='lidar_link']")
        assert lidar_link.find("sensor[@type='gpu_lidar']") is not None

    def test_lidar_noise_block_present(self, robot_model: ET.Element) -> None:
        lidar_link = robot_model.find("link[@name='lidar_link']")
    ...
    def test_lidar_noise_type_gaussian(self, robot_model: ET.Element) -> None:
        lidar_link = robot_model.find("link[@name='lidar_link']")
        sensor = lidar_link.find("sensor[@type='gpu_lidar']")
        noise = sensor.find(".//noise")
        assert noise.find("type").text == "gaussian"

    def test_lidar_noise_mean_zero(self, robot_model: ET.Element) -> None:
        lidar_link = robot_model.find("link[@name='lidar_link']")
        sensor = lidar_link.find("sensor[@type='gpu_lidar']")
        noise = sensor.find(".//noise")
        assert float(noise.find("mean").text) == pytest.approx(0.0)

    def test_lidar_noise_stddev(self, robot_model: ET.Element) -> None:
        lidar_link = robot_model.find("link[@name='lidar_link']")
        sensor = lidar_link.find("sensor[@type='gpu_lidar']")
        noise = sensor.find(".//noise")
        assert float(noise.find("stddev").text) == pytest.approx(RobotSpecs.LIDAR_NOISE_STDDEV)

    def test_lidar_min_range_50mm(self, robot_model: ET.Element) -> None:
        lidar_link = robot_model.find("link[@name='lidar_link']")
        sensor = lidar_link.find("sensor[@type='gpu_lidar']")
    ...
    def test_lidar_max_range(self, robot_model: ET.Element) -> None:
        lidar_link = robot_model.find("link[@name='lidar_link']")
        sensor = lidar_link.find("sensor[@type='gpu_lidar']")
...
class TestImu:
    def test_imu_link_exists(self, robot_model: ET.Element) -> None:
        assert robot_model.find("link[@name='imu_link']") is not None

    def test_imu_sensor_present(self, robot_model: ET.Element) -> None:
        imu_link = robot_model.find("link[@name='imu_link']")
        assert imu_link.find("sensor[@type='imu']") is not None

    def test_imu_angular_velocity_noise_present(self, robot_model: ET.Element) -> None:
        imu_link = robot_model.find("link[@name='imu_link']")
    ...
    def test_imu_angular_velocity_stddev(self, robot_model: ET.Element) -> None:
        imu_link = robot_model.find("link[@name='imu_link']")
        sensor = imu_link.find("sensor[@type='imu']")
        # All three axes should have stddev = IMU_GYRO_NOISE
        for noise in sensor.findall(".//angular_velocity//noise"):
            assert float(noise.find("stddev").text) == pytest.approx(RobotSpecs.IMU_GYRO_NOISE), (
                f"Expected gyro noise {RobotSpecs.IMU_GYRO_NOISE} rad/s"
            )

    def test_imu_linear_acceleration_noise_present(self, robot_model: ET.Element) -> None:
        imu_link = robot_model.find("link[@name='imu_link']")
        sensor = imu_link.find("sensor[@type='imu']")
    ...
    def test_imu_linear_acceleration_stddev(self, robot_model: ET.Element) -> None:
        imu_link = robot_model.find("link[@name='imu_link']")
        sensor = imu_link.find("sensor[@type='imu']")
        for noise in sensor.findall(".//linear_acceleration//noise"):
            assert float(noise.find("stddev").text) == pytest.approx(RobotSpecs.IMU_ACCEL_NOISE), (
...
def test_generated_sdf_is_valid_xml(robot_world: ET.Element) -> None:
...
def test_all_sensor_links_have_fixed_joints(robot_model: ET.Element) -> None:
