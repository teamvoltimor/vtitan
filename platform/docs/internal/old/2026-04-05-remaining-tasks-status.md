> **⚠️ DEPRECATED / ARCHIVED (2026-06-14).** This document marks work "✅ COMPLETED"
> against the old flat `navigation/navigator.py` (methods `_compute_obstacle_correction`,
> `_execute_parking`, `_control_loop`). That navigator is **dead code and has been deleted**;
> the live path is `navigation/core_navigator.py` (`CoreNavigator`), which implements these
> concerns differently. The claims below no longer reflect the codebase and are retained for
> history only. For the current state see
> [`internal/2026-06-13-state-assessment.md`](../2026-06-13-state-assessment.md).
...
- **State Machine Node:** Completed (`robot/ros2_ws/src/voldemorbot_robot/voldemorbot_robot/state_machine_node.py`).
- **OLED Display Node:** Completed (`robot/ros2_ws/src/voldemorbot_robot/voldemorbot_robot/oled_display_node.py`).
