...
class TelemetryBridgeNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self._command_channel_target = self.get_parameter("command_channel_target").value
        ...
        self._button_pub = self.create_publisher(String, "/button/event", 10)
        ...
        self._vision_params_client: Client = self.create_client(SetParameters, "/vision_detector/set_parameters")
    ...
    def _resolve_robot_id(self) -> str | None:
        ...
        try:
            ...
            self.get_logger().warning(f"Could not resolve robot id: {exc}", throttle_duration_sec=30.0)
            return None
        if not robots:
            self.get_logger().warning("Backend has no registered robots yet", throttle_duration_sec=30.0)
            return None
    ...
    def _command_channel_loop(self) -> None:
        ...
        Runs off the executor thread deliberately -- stub.StreamCommands()
        blocks on stream.recv() for the life of the connection, and the same
        stall-the-whole-node problem documented on _http_executor above
        applies here too.
        """
        while not self._command_stream_stop.is_set():
            if self._robot_id is None:
                ...
                if self._robot_id is None:
                    ...
                    continue
                self.get_logger().info(f"Resolved robot id for command channel: {self._robot_id}")

            try:
                self._run_command_stream(self._robot_id)
            except grpc.RpcError as exc:
                self.get_logger().warning(f"Command stream error: {exc.code()} {exc.details()}")
            except Exception as exc:  # noqa: BLE001
                self.get_logger().warning(f"Command stream error: {exc}")
    ...
    def _run_command_stream(self, robot_id: str) -> None:
        ...
        try:
            ...
            self.get_logger().info(f"Opening command stream to {self._command_channel_target}")
    ...
    def _dispatch_set_vision_debug(self, params: commands_pb2.SetVisionDebugParams) -> tuple[int, str]:
        ...
        # this node's executor (main()'s rclpy.spin call) -- not this
    ...
    def _ack_command(
        ...
        try:
            ...
            self.get_logger().warning(f"AckCommand failed: {exc.code()} {exc.details()}")
