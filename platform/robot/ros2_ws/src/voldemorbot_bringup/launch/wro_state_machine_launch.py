...
def generate_launch_description() -> LaunchDescription:
    ...
    voldemorbot_bringup_dir = get_package_share_directory("voldemorbot_bringup")

    # Include LiDAR launch file
    lidar_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(str(Path(voldemorbot_bringup_dir) / "launch" / "lidar_launch.py")),
    )

    # Static TF publishers for sensor frames
    static_tfs = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(str(Path(voldemorbot_bringup_dir) / "launch" / "static_tfs.launch.py")),
    )
