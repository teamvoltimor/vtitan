...
``VISION_DEBUG_VIDEO=1`` in ``platform/robot/.env`` (or ``publish_annotated:=true``
on the node) -- without it the topic has no publisher and this exits saying so
rather than waiting forever.

Usage (on the Pi, with the stack running)::

    pixi run -e vision python scripts/record_vision_video.py --seconds 20 --out /tmp/run.mp4
