...
Usage (from ``platform/robot``, PYTHONPATH=.)::
    ...
    python scripts/diag_sign_pairs.py            # all 16, summary
    python scripts/diag_sign_pairs.py 4 --ticks  # per-tick detail for one
    python scripts/diag_sign_pairs.py --scenarios-dir <corpus> --quiet
...
from diag_sign_sweep import SweepConfig
...
def probe(
    ...
    if router is None:
        return {"label": scenario.label, "skipped": "no router"}
