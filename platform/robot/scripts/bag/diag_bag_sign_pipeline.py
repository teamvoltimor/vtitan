...
def main() -> int:
    ...
    print()
    print("  WHERE the contacts happen (2026-09-09, five obstacles runs): 40% fall")
    print("  within the first 90 deg of TRACK, and 4 of 5 runs take their first hit")
    print("  before 100 deg -- early in the LAP, though not early on the CLOCK: zero")
    print("  contacts in the first 10% of any run, because a bay start barely moves.")
    print()
    print("  But the start is where commitment is BEST, not worst: the first")
    print("  commitment of a run medians 0.81 m against 0.35 m for all later ones.")
    print("  That decline is CHURN, not decay -- commitments outnumber distinct sign")
    print("  locations 4.4x to 22x, and active_sign_count reaches 14 on a track that")
    print("  holds at most 8. So the 0.38 m commit median is mostly RE-commitment at")
    print("  close range, and detector reach is the cleaner of the two losses.")
    print()
    print("  AND WHY the router keeps dropping it: 83% of commitment episodes begin")
    print("  after a DROP rather than a switch (165 of 199), held 0.35-0.70 s then")
    print("  gone for ~0.50 s. Not corridor flapping -- only 12% of drops land within")
    print("  3 ticks of a corridor flip. Not a stale index either: signs are only")
    print("  APPENDED, so published_index is stable. It is the believed POSITION:")
    print("  within a LIVE commitment it moves p50 7.7 cm, p90 60.9 cm, max 103 cm,")
    print("  with 47% of moves past 10 cm. A sign that teleports 60 cm stops being an")
    print("  active candidate, and the deformation target it feeds jumps with it.")
    print()
    print("  The sim does NOT reproduce this: 15 drops over a 3-lap run against 162")
    print("  over five short hardware runs, because its sign map is stable. Isolate")
    print("  this on hardware bags, not in a sweep.")
