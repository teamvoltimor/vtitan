...
JUST_INSIDE_TURN_M = TURN_CLEARANCE_M - TURN_ENTRY_MARGIN_M
...
class TestCornerTurn:
    def test_wall_spanning_the_track_commits_to_the_turn(self) -> None:
        ...
        scan = LidarScanBuilder().corridor(left_m=0.5, right_m=0.5, ahead_m=JUST_INSIDE_TURN_M).build()
        cmd = follow_corridor(scan.ranges, scan.angles, CREEP_SPEED_MPS)
    ...
    def test_turns_toward_the_side_with_more_room(self) -> None:
        ...
        scan = LidarScanBuilder().corridor(left_m=0.9, right_m=0.3, ahead_m=JUST_INSIDE_TURN_M).build()
        assert follow_corridor(scan.ranges, scan.angles, CREEP_SPEED_MPS).steering_norm > 0

        scan = LidarScanBuilder().corridor(left_m=0.3, right_m=0.9, ahead_m=JUST_INSIDE_TURN_M).build()
        assert follow_corridor(scan.ranges, scan.angles, CREEP_SPEED_MPS).steering_norm < 0
    ...
    def test_a_single_dropped_beam_cannot_veto_a_real_corner(self) -> None:
        ...
        cmd = follow_corridor(ranges_with_dropout, scan.angles, CREEP_SPEED_MPS)
        assert abs(cmd.steering_norm) == pytest.approx(_MAX_CENTERING_STEER)
