...
# ~/vtitan_runs/run_<timestamp>/video.mp4 alongside that run's mcap bag) down
# into this folder for local review, WITHOUT the (often much larger) bag.
#
# Deliberately a separate tree from vtitan_runs_pulled/, not a filter added to
...
log "Pulling ${#RUNS[@]} video(s) from $PI5_HOST:$REMOTE_BAG_DIR -> $HERE"
for run in "${RUNS[@]}"; do
  if [ -f "$HERE/$run/video.mp4" ]; then
    log "  $run (already present locally, skipping)"
    continue
  fi
  log "  $run"
  mkdir -p "$HERE/$run"
  scp "${SSH_OPTS[@]}" -q "$PI5_HOST:$REMOTE_BAG_DIR/$run/video.mp4" "$HERE/$run/video.mp4" ||
