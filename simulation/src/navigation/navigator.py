...
class TrackNavigator(Node):
    ...
    def __init__(
        ...
        self._is_open_challenge = (
            self._metadata.get("challenge_type", "open") == "open"
        )
        ...
        start_cond = self._metadata["starting_conditions"]
        self._start_x: float = start_cond["position"]["x"]
        self._start_y: float = start_cond["position"]["y"]
        self._start_yaw: float = start_cond["yaw"]
    ...
    def _enter_wall_escape(
        ...
        wall_limited = max(6, min(12, int(escape_side_dist / 0.03)))
    ...
    def _maybe_advance_waypoint(
        ...
        if distance < self._waypoint_threshold:
            ...
            if self._dist_increasing_count >= 10 and distance > self._waypoint_threshold:
                should_skip = True
