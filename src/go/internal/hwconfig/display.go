...
import (
	...
	driverssd1306 "github.com/teamvoltimor/vtitan/src/go/pkg/driver/display/ssd1306"
)

// Display resolves the Config to Connect with: if configRoot is
...
func Display(logger *slog.Logger, configRoot string) driverssd1306.Config {
	cfg := driverssd1306.DefaultConfig()
	if configRoot == "" {
		return cfg
	}
	...
	if err != nil {
		logger.Warn(
			"hwconfig: loading hardware profile, falling back to default config",
			"config_root",
			configRoot,
			"error",
			err,
	...
	if err != nil {
		logger.Warn(
			"hwconfig: parsing hardware profile's i2c_address, falling back to default config",
			"config_root",
			configRoot,
			"error",
			err,
