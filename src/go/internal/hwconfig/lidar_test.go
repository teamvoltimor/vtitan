...
// writeConfigRoot lays out the two files hwconfig.LIDAR reads -- the serial
...
func TestConfigForResolvesMountCorrection(t *testing.T) {
	...
	cfg := hwconfig.LIDAR(slog.Default(), root)
...
func TestConfigForMountCorrectionNeedsNoHardwareProfile(t *testing.T) {
	...
	if cfg := hwconfig.LIDAR(slog.Default(), root); !cfg.Inverted {
