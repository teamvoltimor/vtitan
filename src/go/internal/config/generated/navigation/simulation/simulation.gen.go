...
type NavigationSimulationSimulation struct {
	...
	// corpus scenario uses the second, so the bay exit the real car spends 23-40 s on
	// is invisible to every sweep in the repo. Ships FALSE, and the reason is a RULE,
	// not a broken manoeuvre: the exit does get out. MEASURED on the committed 16 --
	// with the fin contact forgiven, purely as a control, 10 of 16 reach three laps,
	// the same ten the parallel start clears. Left terminal, the score is 0 of 16,
	// every run ending at 15-23 s on a marker fin, which 9.24.7 ends the round for
	// with no grace and no movement test. So what the in-bay start currently measures
	// is that the exit scrapes the pocket on its way out. The other cost it exposes
	// is time: four of the ten that finish run OVER the 180 s limit, matching the
	// hardware rounds that timed out.
	...
	// Probability that a detection reports the OPPOSITE colour; ships the measured
	// marginal rate, applied i.i.d. per detection.
	...
	// Project the parking lot's magenta blocks into the vision emulator as
	// detections. SHIPS ON and is INERT on the corpus -- 14 failing either way, the
	// same set name for name -- which is the point: it buys coverage for free. The
	// barrier belief is deployed on the car and nothing in simulation exercised it,
	// because the barrier simply did not exist there. MEASURED alongside it, and the
	// reason vision_barrier_red_rate stays at 0.0: at 0.30 the corpus goes 14 -> 48,
	// breaking 34 tests, almost all of them blind discovery. A correctly classified
	// barrier is harmless; one that can be read RED puts a phantom pillar in the map.
	// That is not hypothetical -- run_20260915_140358 carries 1145 red detections of
	// it, 90% of them in the lot's own section -- so the deployed robot operates in
	// the regime the corpus scores at 48 while the corpus itself is scored at 0.0.
	// Raising this knob is therefore realism the bench cannot afford until blind
	// discovery survives a red barrier; the belief shipped for exactly this case is
	// active in that measurement and does not save it.
	...
	// tick, on top of the range model. It is a residual DETECTOR-miss term, not a
	// camera frame-rate term: 15 fps against a 20 Hz control loop would only account
	// for a quarter of it. CALIBRATED 2026-09-16; it shipped at 0.79, which the
	// measurement excludes by a wide margin. The hardware axis is the share of nav
	// ticks carrying a fresh USABLE COLOUR OBSERVATION -- what survives
	// decode_detections AND the pillar aspect gate in
	// navigation.planning.sign_discovery.detection_to_observation, i.e. what
	// production actually keeps, not what the detector emitted: 32.2% / 31.7% / 24.9%
	// on run_20260915_140358 / 140852 / 141413, mean 29.6%. The simulator axis is the
	// share of gateway polls returning a detection, swept over 8 fixtures x 8 values:
	// 0.79 -> 9.3%, 0.60 -> 16.8%, 0.50 -> 22.2%, 0.40 -> 27.9%, 0.30 -> 29.6%, 0.20
	// -> 34.0%, 0.10 -> 36.5%, 0.00 -> 41.0%. So 0.30 is the match; the defensible
	// band is 0.24-0.45 and, under a stricter pairing of the two denominators, as low
	// as 0.10. WHAT THIS KNOB CANNOT REACH: at miss = 0.0 the emulated camera still
	// carries a detection on only 41% of polls. That ceiling is set by
	// vision_detect_r50_m (1.1 m), NOT by this knob -- so the older claim that the
	// range model alone leaves the emulator detecting on ~54.5% of ticks is not
	// reproducible at any value here, and the simulated_hardware_gateway docstring
	// asserting it has been corrected. The 0.79 that shipped was fitted against a
	// different quantity ("hardware carries a detection on 11.6% of ticks", pooled
	// over 125 bags), which is not the surviving-observation share measured above, so
	// the two are not the same axis. THIS IS REALISM, NOT AN IMPROVEMENT: laps>=3
	// moves NON-MONOTONICALLY across that ladder, so the change RE-BASES the
	// Obstacles corpus. A failure count taken at 0.79 and one taken at 0.30 are not
