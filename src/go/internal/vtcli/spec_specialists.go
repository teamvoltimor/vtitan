...
// Task's own default, an ambiguous alias, and the per-service lint and format
// leaves the aggregate verbs already cover. Everything else is wrapped above.
...
// hailoExclusions are the hailo tasks left out of the tree: only Task's own
// default, since the Docker suite and the pipelines are wrapped above.
