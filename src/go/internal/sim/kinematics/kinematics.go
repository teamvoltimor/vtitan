...
func (k *AckermannKinematics) Step(state AckermannState, targetSpeed, targetSteerNorm, dt float64) AckermannState {
	targetSteer := geom.Clamp(targetSteerNorm, -control.SteeringNormLimit, control.SteeringNormLimit) * k.maxSteer
	...
	for i := 0; i < k.substeps; i++ {
		...
		steer = geom.Clamp(steer, -k.maxSteer, k.maxSteer)
		...
		setpoint := geom.Clamp(targetSpeed, -k.maxSpeed, k.maxSpeed)
		lagged := setpoint
		if k.speedTauS > 0 {
