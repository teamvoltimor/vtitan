...
const (
	// cdrU32Size is the alignment unit of a CDR uint32/int32/float32.
	cdrU32Size = 4
	// cdrF64Size is the alignment unit of a CDR float64.
	cdrF64Size = 8
	// covarianceLen is the fixed element count of a ROS covariance array.
	covarianceLen = 9
)
...
func EncodeImu(imu ImuCDR) []byte {
	...
	w.writeF64Array(make([]float64, covarianceLen))
	...
	// accelerometer.
	unknown := make([]float64, covarianceLen)
	unknown[0] = -1
