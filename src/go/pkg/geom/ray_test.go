...
func TestNearestRay(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got := geom.NearestRay(
				geom.LidarScan{RangesM: ranges, AnglesRad: angles},
				tt.target,
			); got != tt.want {
...
func TestForwardClearance(t *testing.T) {
	...
	t.Run("minimum within arc", func(t *testing.T) {
		...
		got := geom.ForwardClearance(geom.LidarScan{RangesM: ranges, AnglesRad: angles}, arcRad, minValid)
		if got != 1.0 {
