...
// On the Pi Zero both the RPWM hardware-PWM sysfs channel and the LPWM
// software-PWM channel (pkg/driver/motor's sysfs_pwm.go and soft_pwm.go)
// implement it identically from Controller's point of view - it never
// needs to know which physical channel it's talking to. Defined here, where
// it's consumed, per go-architect §4.
...
// (pkg/driver/motor's gpio_enable.go on the Pi Zero).
...
// real GPIO or /sys/class/pwm access. pkg/driver/motor's driver.go wires it
// to real hardware adapters.
...
var ErrNotConnected = errors.New("hbridge: SetSpeed called before Connect")
...
func (c *Controller) Connect(ctx context.Context) error {
	...
	if err := c.rpwm.SetDuty(ctx, dutyOff); err != nil {
		return fmt.Errorf("hbridge: zeroing RPWM before enable: %w", err)
	}
	if err := c.lpwm.SetDuty(ctx, dutyOff); err != nil {
		return fmt.Errorf("hbridge: zeroing LPWM before enable: %w", err)
	}
	...
	if err := c.rEn.SetHigh(ctx, true); err != nil {
		return fmt.Errorf("hbridge: enabling R_EN: %w", err)
	}
	if err := c.lEn.SetHigh(ctx, true); err != nil {
		return fmt.Errorf("hbridge: enabling L_EN: %w", err)
	}
...
func (c *Controller) SetSpeed(ctx context.Context, normalizedSpeed float64) error {
	...
	if rpwmDuty == dutyOff {
		if err := c.rpwm.SetDuty(ctx, dutyOff); err != nil {
			return fmt.Errorf("hbridge: zeroing RPWM: %w", err)
		}
		if err := c.lpwm.SetDuty(ctx, lpwmDuty); err != nil {
			return fmt.Errorf("hbridge: setting LPWM duty: %w", err)
		}
		return nil
	}

	if err := c.lpwm.SetDuty(ctx, dutyOff); err != nil {
		return fmt.Errorf("hbridge: zeroing LPWM: %w", err)
	}
	if err := c.rpwm.SetDuty(ctx, rpwmDuty); err != nil {
		return fmt.Errorf("hbridge: setting RPWM duty: %w", err)
	}
	return nil
}
...
// LPWM, not yet installed as of this port. pkg/driver/motor's Close adds a
...
func (c *Controller) Close(ctx context.Context) error {
	...
	if err := c.rpwm.SetDuty(ctx, dutyOff); err != nil {
		errs = append(errs, fmt.Errorf("hbridge: zeroing RPWM on close: %w", err))
	}
	if err := c.lpwm.SetDuty(ctx, dutyOff); err != nil {
		errs = append(errs, fmt.Errorf("hbridge: zeroing LPWM on close: %w", err))
	}
	if err := c.rEn.SetHigh(ctx, false); err != nil {
		errs = append(errs, fmt.Errorf("hbridge: disabling R_EN on close: %w", err))
	}
	if err := c.lEn.SetHigh(ctx, false); err != nil {
		errs = append(errs, fmt.Errorf("hbridge: disabling L_EN on close: %w", err))
	}
