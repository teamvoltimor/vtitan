...
whether a result survives the error budget the robot actually has; it is the
closest simulation gets to hardware validation for anything except the
understeer, which no sensor model reaches. See
...
REAL_SENSOR_ERRORS = SensorErrors(
    ...
    # so this is the residual that survives the measurement, not the old
    # assumption's much larger error; see
