...
def main() -> int:
    ...
    for i, (widths, section, direction, cell) in cases:
        ...
        by_verdict[outcome] += 1
        dims["por seccion de salida:"][(section.value, outcome)] += 1
        dims["por direccion:"][(direction.value, outcome)] += 1
        dims["por celda de salida:"][(f"cell {cell}", outcome)] += 1
        dims["por ancho del pasillo de salida:"][(f"{widths_mm[section.value.lower()]}mm", outcome)] += 1

        label = f"{'-'.join(str(w) for w in widths)} {section.value}/{direction.value} c{cell}"
        line = (
            f"{'OK  ' if outcome == 'ok' else 'FAIL'} [{i:>3}] {label:<46} "
            f"{outcome:<10} laps={result.laps_completed}/{result.target_laps} "
