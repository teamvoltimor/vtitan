...
#   3. Copy src (which nests shared/, pulled in via pixi.toml's
#      `vtitan-shared = { path = "./shared" }`) onto the Zero --
#      git archive over SSH, no GitHub auth needed on the Zero itself
...
log "3/7 Copying src/python (with nested shared/) onto the Zero (git archive over SSH, no auth needed on the Zero)"
cd "$ROBOT_DIR/../.."  # repo root
git archive HEAD -- src/python | ssh "${SSH_OPTS[@]}" "$ZERO_HOST" "mkdir -p ~/vtitan && tar -x -C ~/vtitan"
cd "$ROBOT_DIR"

log "4/7 Creating .env from .env.example on the Zero (only if missing -- never overwrites)"
ssh "${SSH_OPTS[@]}" "$ZERO_HOST" "cd ~/vtitan/src/python && [ -f .env ] || cp .env.example .env"
...
ssh "${SSH_OPTS[@]}" "$ZERO_HOST" "
  sed -e 's|__TARGET_USER__|$ZERO_USER|g' -e 's|__TARGET_HOME__|$ZERO_HOME|g' \
    ...
    ~/vtitan/src/python/systemd/vtitan-pi-zero.service \
