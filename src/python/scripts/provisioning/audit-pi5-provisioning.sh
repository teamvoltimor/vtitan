...
[ -f "$HOME/vtitan/src/python/.env" ] && ok ".env present" || bad ".env missing"
...
for setting in net.core.rmem_max net.core.rmem_default net.core.wmem_max net.core.wmem_default; do
  ...
  [ "$val" = "8388608" ] && ok "$setting = 8388608" || bad "$setting not raised (got: $val) -- Pi5<->Zero DDS delivery will silently fail under load, see other/deploy/ansible/roles/common/tasks/main.yml"
done
...
[ -d "$HOME/vtitan/src/python/ros2_ws/src/sllidar_ros2" ] && ok "sllidar_ros2 driver fetched" || bad "sllidar_ros2 driver not found (check ros2_ws/src path if this looks wrong)"
[ -d "$HOME/vtitan/src/python/ros2_ws/install" ] && ok "ROS2 workspace built (install/ exists)" || bad "ROS2 workspace not built"

section "pi5 role: mise + Go + backend"
[ -x "$HOME/.local/bin/mise" ] && ok "mise installed" || bad "mise missing"
[ -x "$HOME/vtitan/apps/backend/bin/server" ] && ok "telemetry backend binary built" || bad "telemetry backend binary missing"
...
systemctl is-enabled vtitan-boot-check-watchdog.timer >/dev/null 2>&1 && ok "boot-check watchdog timer enabled" || bad "boot-check watchdog timer not enabled -- a stuck BOOT_CHECK (matched-but-silent DDS) will never self-recover"
