...
grep -q '^MOTOR_TEST_DURATION=' "$HOME/vtitan/src/python/.env" 2>/dev/null && ok "MOTOR_TEST_DURATION set" || bad "MOTOR_TEST_DURATION missing from .env -- ackermann_motor_node's Config() throws uncaught, node gets stuck permanently unconfigured (looks like a wiring/pin problem, isn't -- confirmed on hardware 2026-08-09)"
...
[ -d "$HOME/vtitan/src/python/ros2_ws/install" ] && ok "ROS2 workspace present (shipped from Pi 5)" || bad "ROS2 workspace missing -- deploy-dev-env-to-zero.sh has not run/succeeded"
