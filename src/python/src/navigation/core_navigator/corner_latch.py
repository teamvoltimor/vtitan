...
thresholding it un-arms the short lookahead mid-turn.

The other two demands cannot cover the gap. ``crosstrack_error`` is measured
against the planned path and stays small right through the corner -- the chassis
is ON the path, it is POINTING off it -- and ``sign_ahead`` is an Obstacles-only
boolean. ``select_lookahead``'s own docstring records the earlier form of this
failure (commanding a small fraction of lock while still on-path); previewing
the turn fixed it on approach, and this fixes the same thing once the robot is
inside the corner.
...
Measured rationale: ``adr:0052-pursuit-target-selection``.
"""
