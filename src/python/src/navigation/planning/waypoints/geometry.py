...
def corner_arc_radius(
    ...
    The shipped biases are ``wide_center_bias_m = 0.10``, ``narrow_center_bias_m
    = 0.0`` and ``obstacles_center_bias_m = 0.15``. ``CORNER_ARC_ASSUME_WIDE``
    (shipped true) passes ``WIDE`` for BOTH widths while
    ``center_bias_for_corridor`` still picks the bias from the corridor's TRUE
    class, so the width term is a constant 1.00 m and only the bias varies::
    ...
    Two consequences. ``arc_radius = 0.45`` is INERT wherever a bias applies --
    only the narrow class still reaches it. And because the width is forced wide
    while the bias is not, the WIDE corridor receives the TIGHTER arc of the
    two, 0.40 against 0.45. That inversion is the first link in the Open zigzag
    chain (tighter arc -> shorter straight between arcs -> the short lookahead
    covers most of the straight -> higher loop gain), not a rounding detail. See
    ``adr:0049-corner-arcs-per-corridor-and-commit-distance`` and
