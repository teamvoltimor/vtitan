    ...
    def parse_yolo_nms_output(
        ...
        Args:
            ...
            conf_threshold: Minimum confidence threshold to filter detections.
                Required, not defaulted: the caller owns the config that resolves
                it (detector.toml ``min_confidence`` via HailoConfig), and a
                hidden fallback load here would re-read that TOML behind the
                caller's back.
