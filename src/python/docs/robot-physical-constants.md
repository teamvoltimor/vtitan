...
**Single source of truth (2026-07-11 fix):** `src/shared/config/robot.toml`. It used to
...
**To change a measurement:** edit `src/shared/config/robot.toml`, then hand-edit the two
...
- `src/go/internal/simgen/simconfig/robot_constants.gen.go` (Go)
- `apps/gazebo/runtime/robot_description/robot_properties.gen.xacro` (`xacro:include`d from
  `wro_robot.urdf.xacro`)

Python has no generated file to regenerate: `RobotSpecs` in
`src/shared/src/shared/config/constants.py` sources its values from
...
| Camera mount z-offset | 0.20 m | approximate measurement (2026-09-07) |
| Camera mount pitch | ~10° down | Approximate measurement (2026-09-07). Sign convention (positive = down) is confirmed correct - both by a rotation-matrix derivation (`R = Rz(yaw)·Ry(pitch)·Rx(roll)`, standard REP-103/tf2 convention) and visually, via the yellow direction-arrow marker added to `src/simulation/live_visualizer.py` and checked live in RViz 2026-07-11. |
...
### Source of truth
`src/shared/config/robot.toml` - chassis, Ackermann geometry, wheel, LIDAR mount offset,
...
### Generated (no longer regenerated - see above; edit by hand and keep in sync)
- `src/go/internal/simgen/simconfig/robot_constants.gen.go` - the `Robot*` const
  block, consumed by `src/go/internal/simgen/sdf/robot.go` (chassis box/inertia,
...
- `apps/gazebo/runtime/robot_description/robot_properties.gen.xacro` - `xacro:include`d
...
`src/shared/src/shared/config/constants.py` sources its fields (`LENGTH`, `WIDTH`,
...
- `apps/gazebo/runtime/robot_description/wro_robot.urdf` - a **separate, simplified,
...
camera (pitch = 0). With the camera's mount pitch now confirmed non-zero (~10° down), the
...
Done (2026-07-11): `src/shared/config/robot.toml` is now the shared source all three read.
