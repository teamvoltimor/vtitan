...
Lands in `src/.corpus/` - gitignored, regenerated, byte-identical
...
`src/shared/config/navigation-challenges/obstacles/` overlay, never
through `src/shared/config/navigation/` (the base config Open Challenge
...
Run from `src` with `PYTHONPATH=.` under `pixi run -e dev`.
