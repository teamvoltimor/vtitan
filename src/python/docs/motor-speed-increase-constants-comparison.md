...
0.156` in `src/shared/config/robot.toml`. Projected column assumes the
...
|---|---|---|---|---|
| `max_speed_mps` | `src/shared/config/robot.toml` [drivetrain] | 0.156 | ~0.25 (re-measure) | Hard ceiling - `AckermannKinematics` clamps to it. Single source; everything below reads through `RobotSpecs.MAX_SPEED_MPS`. |
