...
def connect_to_model_server(
    ...
    Args:
        ...
        server_cfg: Server configuration for host/port. Falls back to AppConfig when omitted.
    ...
    if server_cfg is None:
        ...
        server_cfg = AppConfig.load().server

    server_port = server_cfg.port
    server_port_source = server_cfg.port_source

    deadline = time.monotonic() + timeout
    attempt = 0

    while time.monotonic() < deadline:
        candidate = ModelServerClient(host=server_cfg.host, port=server_port, recv_chunk_size=server_cfg.recv_chunk_size)
        if candidate.ping():
            logger.info(
