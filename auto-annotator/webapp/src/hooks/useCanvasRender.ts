...
export const useCanvasRender = (annotationPoints: AnnotationPoint[], segmentationPreview: SegmentationPreviewShape[]) => {
  ...
  const ensureCanvasResolution = () => {
    ...
    return { rectWidth: rect.width, rectHeight: rect.height, canvasWidth: pixelWidth, canvasHeight: pixelHeight, dpr }
  }
  ...
  const drawCanvas = useCallback(
    (ctx: CanvasRenderingContext2D, image: HTMLImageElement) => {
      ...
      const margin = 80 * dpr
      const scale = Math.min((canvasWidth - margin) / image.width, (canvasHeight - margin) / image.height)
