...
type fakeClients struct {
	...
	segResult compute.SegmentResult
}

func (f *fakeClients) Segment(context.Context, compute.SegmentInput) (compute.SegmentResult, error) {
	return f.segResult, f.segErr
}

func (f *fakeClients) RunAugmentation(
	_ context.Context,
	_ compute.AugmentInput,
	onProgress func(compute.Progress),
) error {
	if f.augBlock != nil {
		<-f.augBlock
	}
	onProgress(compute.Progress{Status: "running", Message: "augmenting", Progress: 1})
	return nil
}

func (f *fakeClients) RunTraining(context.Context, compute.TrainInput, func(compute.Progress)) error {
	return nil
}

func (f *fakeClients) Close() error { return nil }

func newTestApp(t *testing.T, fc compute.Clients) *App {
	t.Helper()
...
func TestSegment(t *testing.T) {
	fc := &fakeClients{segResult: compute.SegmentResult{
		State:  string(compute.StateReady),
		Shapes: []compute.Shape{{ID: "mask-1-0", ClassName: "red_prism", Points: []compute.Point{{X: 0.1, Y: 0.2}}}},
	}}
	app := newTestApp(t, fc)
	if err := app.Store.Q.InsertImageOrIgnore(context.Background(), "/tmp/x.png"); err != nil {
	...
	t.Run("ready", func(t *testing.T) {
		...
		if resp.State != string(compute.StateReady) || len(resp.Shapes) != 1 ||
	...
	t.Run("missing image -> 400", func(t *testing.T) {
		rec := doJSON(t, router, http.MethodPost, "/api/v1/segment",
...
func TestAugmentJobLifecycle(t *testing.T) {
	...
	for app.Jobs.Running() && time.Now().Before(deadline) {
