...
--
-- This mirrors the final state produced by the Python/Alembic migrations
-- (001_initial + 002_schema_updates). It is read by sqlc for code generation
-- and applied at runtime with CREATE TABLE IF NOT EXISTS, so it is a no-op
-- against an existing database created by the Python side and creates the
-- tables for a fresh Go-owned database.
--
