#!/usr/bin/env bash
# Promote a compiled model from ml/hailo/shared_with_docker/ (the compiler's
# scratch directory) into a new tracked models/<name>/vN/.
#
# The .hef file must be named explicitly, not guessed: ml/hailo/shared_with_docker/
...
#
# Usage: bash ml/weights/scripts/promote-from-hailo.sh <name> <hef-file> [onnx-file]
#   bash ml/weights/scripts/promote-from-hailo.sh gmr gmr_cpu_opt0.hef gmr.onnx
...
SRC_DIR="$REPO_DIR/ml/hailo/shared_with_docker"
MODEL_DIR="$HERE/$NAME"
...
model: $NAME
version: $new_version
source: ml/hailo/shared_with_docker/$HEF_FILE
$([ -n "$ONNX_FILE" ] && echo "source_onnx: ml/hailo/shared_with_docker/$ONNX_FILE")
notes: >-
