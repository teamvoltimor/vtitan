...
con buen mensaje, un experimento fallido registrado y un ADR actualizado en
`other/docs/adr/` valen más para el puntaje que una redacción pulida escrita de memoria tres
semanas después.
...
ADRs en `other/docs/adr/`) en una explicación legible para un jurado que no conoce el
código.

Para la parte de ajuste de parámetros, la referencia versionada es
[`configuracion-toml-navegacion.md`](../configuracion-toml-navegacion.md): documenta qué variable
...
1. **Cada decisión de diseño o hallazgo relevante** (mecánico, eléctrico, de software o de
   ...
   - Decisión de arquitectura/algoritmo → `other/docs/adr/` (fuente canónica) o el journal.
...
2. **Los commits sirven de bitácora cruda.** Un buen mensaje de commit (ver
   `feedback_commit_style`: logging estructurado, nunca `print()`) es
...
3. **Cada 2–3 semanas, sesión de "traducción"**: tomar las notas técnicas acumuladas
   (memoria de sesiones de desarrollo, `other/docs/adr/`, commits) y convertir las que tengan
...
- Este archivo vive en `other/docs/documentacion-ingenieria-wro.md` y es una guía de
  proceso, no la documentación final de competencia.
...
- La fecha límite de documentación y el enlace exacto del repositorio que se entregará deben quedar
  registrados aparte (ej. en el journal o en `other/docs/`), no en esta guía.

---

## 10. Qué le falta a la bitácora antes de entregar

*(Movido desde `docs/bitacora-ingenieria.md` el 2026-09-09: es planificación de producción,
no contenido de ingeniería, y la bitácora la ve el jurado.)*
...
y los tags de versión que van pidiendo las secciones de la bitácora.

Hay dos entradas que faltan por escribir. Una es la de estacionamiento, de la que hoy solo
se cuenta la regla de contacto. La otra es la de visión para señales, que se queda en la
corrección de la cámara invertida cuando en realidad hay bastante más. Y a la migración a Go
le falta la entrada que importa: por qué Go y no seguir en Python/ROS2, con datos de
rendimiento, de consumo de recursos en la Pi Zero y de concurrencia, en lugar del relato de
cómo se ejecutó.

Queda además una tarea de lenguaje. Varias entradas usan jerga interna ("sweep", "arm",
nombres de constantes del código) que hay que traducir a algo legible para el jurado sin
perder precisión, y conviene hacerlo con el equipo delante para no cambiar el significado por
el camino.
...
el markdown a PDF o preparar un documento aparte) y si se traduce o se resume al inglés.

Y una comprobación final sobre el README: que explique de punta a punta cómo instalar,
simular y desplegar a hardware sin dar por sentado contexto que solo tiene el equipo.
